...
| `supabase/migrations/` | Sumber kebenaran skema DB (tabel + RLS + storage bucket), dijalankan via `supabase db push`. README §1 berisi SQL yang sama persis untuk setup manual, tapi migration files ini yang otoritatif — kalau ada perbedaan, migration yang benar. |
...
- **`supabase/migrations/` ditambahkan supaya repo ini bisa dipakai sebagai template proyek baru** — sebelumnya skema cuma didokumentasikan sebagai blok SQL di README yang harus di-copy-paste manual satu-satu ke SQL Editor, gampang ke-skip (persis penyebab RLS drift `orders` di atas). Sekarang `supabase db push` sekali jalan bikin semua tabel + RLS + storage bucket sesuai skema final saat ini, tanpa risiko ada policy yang kelewat. Migration files ini jadi sumber kebenaran; SQL di README §1 tetap dipertahankan identik sebagai alternatif manual (buat yang belum mau install CLI), bukan didobel-maintain secara independen.
